...
def main(n_charges=10, ring_radius=1.0, charge_ring=1.0e-9, 
    ...
    with open(f"data/{filename}_params.txt", "w") as param_file:
        param_file.write("# Parámetros de la simulación\n")
        param_file.write(f"n_charges = {n_charges}\n")
        param_file.write(f"ring_radius = {ring_radius} m\n")
        param_file.write(f"charge_ring = {charge_ring} C\n")
        param_file.write(f"electron_position = [{electron_position[0]}, {electron_position[1]}, {electron_position[2]}] m\n")
        param_file.write(f"electron_velocity = [{electron_velocity[0]}, {electron_velocity[1]}, {electron_velocity[2]}] m/s\n")
        param_file.write(f"dt = {dt} s\n")
        param_file.write(f"t_max = {t_max} s\n")
        param_file.write(f"electron_mass = {system.electron_mass} kg\n")
        param_file.write(f"electron_charge = {system.electron_charge} C\n")
        param_file.write(f"k (Coulomb) = {system.k} N·m²/C²\n")
